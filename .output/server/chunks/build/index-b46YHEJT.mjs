...
import { u as useI18n, a as useLocalePath, b as useContentLocale, p as projectInfo, f as useAsyncData, s as supabase, d as useBranding } from './server.mjs';
import { l as loadCategoriesForLocale, _ as _sfc_main$1, c as countLogicalPosts } from './PostLoader-DXP-J1UD.mjs';
import { Icon } from '@iconify/vue';
import { f as fetchStatsSettings, u as useStatsSettings } from './statsSettingsStore-DS17idX8.mjs';
import { f as fetchSettings, u as useSettings } from './settingsStore-B1-8Nave.mjs';
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    const localizedSiteName = computed(
      () => branding.resolveLocalizedSiteName(locale.value) || projectInfo.name || "Pluma"
    );
    const localizedSiteDescription = computed(
      () => branding.resolveLocalizedSiteDescription(locale.value) || projectInfo.description || ""
    );
    useRouter();
    usePageSeo(
      computed(() => ({
        title: localizedSiteName.value,
        description: localizedSiteDescription.value || void 0,
        type: "website"
      }))
    );
    ...
    const { data: homeBootstrap } = ([__temp, __restore] = withAsyncContext(async () => useAsyncData(
      ...
      async () => {
        ...
        const { data: latest } = await supabase.from("posts").select("title, slug, created_at").eq("status", "published").eq("locale", contentLocale.value).order("created_at", { ascending: false }).limit(1).maybeSingle();
        return { categories: cats || [], latestPost: latest || null };
      },
    ...
    async function loadStats() {
      ...
      const [{ count: postsCount }, { count: catCount }, { count: authorCount }] = await Promise.all([
        statsEnabled.value.posts ? countLogicalPosts(
          supabase,
          (q) => q.eq("status", "published").eq("locale", contentLocale.value)
        ) : Promise.resolve({ count: null }),
        statsEnabled.value.categories ? supabase.from("categories").select("id", { count: "exact", head: true }).eq("locale", contentLocale.value) : Promise.resolve({ count: null }),
        statsEnabled.value.authors ? supabase.from("profiles").select("id", { count: "exact", head: true }).in("role", ["author", "admin"]) : Promise.resolve({ count: null })
      ]);
      stats.value = [
        { key: "posts", label: t("home.stats.posts"), value: postsCount != null ? postsCount : "\u2014" },
        { key: "categories", label: t("home.stats.categories"), value: catCount != null ? catCount : "\u2014" },
        { key: "authors", label: t("home.stats.authors"), value: authorCount != null ? authorCount : "\u2014" }
      ];
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (showSiteName.value) {
        _push(`<h1 class="text-4xl md:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-white dark:via-gray-100 dark:to-white">${ssrInterpolate(localizedSiteName.value)}</h1>`);
      } else {
        _push(`<!---->`);
      }
      if (showSiteDescription.value) {
        _push(`<p class="text-lg md:text-xl text-gray-600 dark:text-gray-300 leading-relaxed">${ssrInterpolate(localizedSiteDescription.value)}</p>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</div>`);
...
//# sourceMappingURL=index-b46YHEJT.mjs.map
