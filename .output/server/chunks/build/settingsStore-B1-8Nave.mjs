...
function providerLabel(key) {
  var _a;
  return ((_a = PROVIDER_META[key]) == null ? void 0 : _a.label) || key;
}
function providerIcon(key) {
  var _a;
  return ((_a = PROVIDER_META[key]) == null ? void 0 : _a.icon) || "mdi:link";
}
function brandBg(key) {
  var _a;
  return ((_a = PROVIDER_META[key]) == null ? void 0 : _a.bg) || "#f3f4f6";
}
function brandBorder(key) {
  var _a;
  return ((_a = PROVIDER_META[key]) == null ? void 0 : _a.border) || "#e5e7eb";
}
function providerGlyphColor(key) {
  var _a;
  return ((_a = PROVIDER_META[key]) == null ? void 0 : _a.glyph) || null;
}
async function fetchSettings() {
  const { data: featuresData, error: featuresError } = await supabase.from("settings").select("value").eq("key", "features_home_enabled").maybeSingle();
  ...
  const { data: provData, error: provError } = await supabase.from("settings").select("value").eq("key", "auth_providers_enabled").maybeSingle();
...
export { ALL_PROVIDERS as A, fetchSettings as f, useSettings as u };
//# sourceMappingURL=settingsStore-B1-8Nave.mjs.map
