...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const displayName = computed(() => {
      var _a;
      return isUncategorized.value ? "Uncategorized" : ((_a = category.value) == null ? void 0 : _a.name) || route.params.slug;
    });
    ...
    async function fetchCategory() {
      ...
      if (!isUncategorized.value) {
        let { data, error } = await supabase.from("categories").select("id, name, slug").eq("slug", route.params.slug).single();
        if (error || !data) {
          const fallback = await supabase.from("categories").select("id, name, slug").ilike("slug", route.params.slug).maybeSingle();
        ...
        if (!error && data) {
          category.value = data;
          const { count } = await supabase.from("posts").select("id", { count: "exact", head: true }).eq("status", "published").eq("category_id", data.id);
          postCount.value = typeof count === "number" ? count : 0;
        }
      } else {
        const { count } = await supabase.from("posts").select("id", { count: "exact", head: true }).eq("status", "published").is("category_id", null);
        postCount.value = typeof count === "number" ? count : 0;
      }
      loading.value = false;
    }
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (loading.value) {
        _push(`<div class="flex flex-col items-center text-center gap-8 animate-pulse"><div class="w-20 h-20 rounded-2xl bg-gray-200 dark:bg-gray-700"></div><div class="space-y-4 max-w-2xl w-full"><div class="h-10 w-64 mx-auto rounded bg-gray-200 dark:bg-gray-700"></div><div class="flex justify-center gap-3"><div class="h-5 w-24 rounded bg-gray-200 dark:bg-gray-700"></div><div class="h-5 w-20 rounded bg-gray-200 dark:bg-gray-700"></div></div></div><div class="w-full max-w-xl h-12 rounded-xl bg-gray-200 dark:bg-gray-700"></div></div>`);
      } else {
        ...
        if (postCount.value !== null) {
          ...
          _push(` ${ssrInterpolate(postCount.value)} ${ssrInterpolate(postCount.value === 1 ? "Post" : "Posts")}</span>`);
        } else {
          _push(`<!---->`);
        }
        if (isUncategorized.value) {
...
//# sourceMappingURL=_slug_-D9EhAq4k.mjs.map
