import { ref, computed, unref, watch, resolveComponent, mergeProps, withCtx, createVNode, toDisplayString, createTextVNode, openBlock, createBlock, createCommentVNode, Fragment, renderList, withModifiers, withDirectives, vModelText, withKeys, reactive, useModel, mergeModels, nextTick, useSSRContext } from 'vue';
...
import { _ as _export_sfc, u as useI18n, a as useLocalePath, C as CONTENT_LOCALES, j as fetchBranding, s as supabase, d as useBranding, b as useContentLocale, D as DEFAULT_FOOTER_CREDITS, n as normalizeLocaleSettings } from './server.mjs';
...
import { _ as _sfc_main$g } from './ConfirmDialog-BMJpAaMk.mjs';
import { Dialog, DialogPanel, DialogTitle, DialogDescription } from '@headlessui/vue';
import { u as useToast } from './useToast-DuA5bmqL.mjs';
import { u as useStatsSettings } from './statsSettingsStore-DS17idX8.mjs';
...
const _sfc_main$e = {
  ...
  emits: ["delete", "edit", "update:selected"],
  setup(__props, { emit: __emit }) {
    const { t, locales } = useI18n();
    const localePath = useLocalePath();
    const props = __props;
    const emit = __emit;
    const activeLocales = reactive(/* @__PURE__ */ Object.create(null));
    ...
    function viewed(post) {
      ...
      const version = (_a = post.translations) == null ? void 0 : _a[code];
      if (!version) {
        return {
          id: post.id,
          title: post.title,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!__props.posts.length) {
        ...
        ssrRenderList(__props.posts, (post) => {
          var _a, _b;
          _push(`<li class="group flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 px-5 sm:px-6 py-4 hover:bg-gray-50/80 dark:hover:bg-gray-900/40 transition-colors"><div class="flex items-start gap-3 min-w-0 flex-1"><input type="checkbox"${ssrRenderAttr("value", viewed(post).id)}${ssrIncludeBooleanAttr(Array.isArray(selectedProxy.value) ? ssrLooseContain(selectedProxy.value, viewed(post).id) : selectedProxy.value) ? " checked" : ""} class="mt-1"${ssrRenderAttr("aria-label", `Select ${viewed(post).title}`)}><div class="min-w-0 flex-1">`);
          _push(ssrRenderComponent(_component_router_link, {
            to: unref(localePath)(`/posts/${viewed(post).slug}`, viewed(post).locale),
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`${ssrInterpolate(viewed(post).title)}`);
              } else {
                return [
                  createTextVNode(toDisplayString(viewed(post).title), 1)
                ];
              }
            }),
            _: 2
          ...
          if ((_b = post.author) == null ? void 0 : _b.username) {
            _push(ssrRenderComponent(_component_router_link, {
              to: unref(localePath)(`/author/${post.author.username}`),
              class: "inline-flex items-center gap-1 hover:text-blue-600 dark:hover:text-blue-400"
            }, {
              default: withCtx((_, _push2, _parent2, _scopeId) => {
          ...
          if (postLocales(post).length) {
            ...
            ssrRenderList(postLocales(post), (code) => {
              _push(`<button type="button" class="${ssrRenderClass([
                activeLocale(post) === code ? "bg-blue-600 text-white dark:bg-blue-500" : "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/50",
                "inline-flex items-center gap-1 h-7 px-2.5 rounded-full text-[11px] font-medium uppercase tracking-wide transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
              ])}"${ssrRenderAttr("title", localeLabel(code))}${ssrRenderAttr("aria-pressed", activeLocale(post) === code)}>`);
              _push(ssrRenderComponent(unref(Icon), {
                icon: "mdi:translate",
                class: "text-sm"
              }, null, _parent));
              _push(` ${ssrInterpolate(code)}</button>`);
            });
            _push(`<!--]--></div>`);
          } else {
            _push(`<!---->`);
          }
          _push(`<span class="${ssrRenderClass([statusClasses(viewed(post).status), "inline-flex items-center gap-1.5 h-7 px-2.5 rounded-full text-[11px] font-medium capitalize"])}"><span class="${ssrRenderClass([dotClasses(viewed(post).status), "w-1.5 h-1.5 rounded-full"])}"></span> ${ssrInterpolate(unref(t)(`posts.status.${viewed(post).status}`, viewed(post).status))}</span>`);
          if (canManage(post)) {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const groups = computed(() => {
      ...
      return [...map.values()].sort(
        (a, b) => groupDisplayName(a).localeCompare(groupDisplayName(b))
      );
    });
    ...
    function groupDisplayName(group) {
      const preferred = group.rows.find((r) => r.locale === manageLocale.value) || group.rows.find((r) => r.locale === primaryLocale.value) || group.rows[0];
      return (preferred == null ? void 0 : preferred.name) || group.slug || "\u2014";
    }
    ...
    function localeChipClass(group, code) {
      if (hasLocale(group, code)) {
        return code === manageLocale.value ? "bg-blue-600 text-white" : "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300";
      }
    ...
    function localeChipTitle(group, code) {
      if (hasLocale(group, code)) {
        ...
        return (row == null ? void 0 : row.name) || code;
      }
    ...
    async function saveTranslation() {
      ...
      try {
        ...
        if (!first.translation_group_id) {
          ...
          await supabase.from("categories").update({ translation_group_id: groupId }).eq("id", first.id);
        }
        ...
        toast.error((e == null ? void 0 : e.message) || t("categories.translationFailed"));
      } finally {
        translating.value = false;
      }
    }
    async function fetchCategories() {
      const { data, error } = await supabase.from("categories").select("id, name, slug, locale, translation_group_id").order("name", { ascending: true });
    ...
    async function addCategory() {
      ...
      let i = 1;
      while (categories.value.some((c) => c.locale === loc && c.slug === uniqueSlug)) {
    ...
    async function updateCategory(id) {
      ...
      if (categories.value.some(
        (c) => c.id !== id && c.locale === (existing == null ? void 0 : existing.locale) && c.name.toLowerCase() === name.toLowerCase()
      )) {
        toast.error(t("categories.exists"));
        return;
      }
    ...
    async function confirmDeleteLocale() {
      ...
      if (!(row == null ? void 0 : row.id) || deletingLocale.value) return;
    ...
    async function confirmDeleteAllLanguages() {
      ...
      try {
        ...
        if (((_a = translateFor.value) == null ? void 0 : _a.groupKey) === group.key) {
          cancelTranslation();
        }
        cancelEdit();
        await fetchCategories();
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps(_ctx.$attrs, _attrs))}><button type="button" class="inline-flex items-center justify-center gap-2 h-9 px-4 rounded-md text-sm font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-900/60 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-auto">`);
      _push(ssrRenderComponent(unref(Icon), {
      ...
      _push(ssrRenderComponent(unref(Dialog), {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(ssrRenderComponent(unref(DialogPanel), { class: "w-full max-w-lg overflow-auto rounded-2xl bg-white dark:bg-gray-800 p-4 sm:p-6 text-start shadow-xl max-h-[90vh]" }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  ...
                  _push3(`<button type="button" class="inline-flex items-center justify-center h-8 w-8 rounded-md bg-gray-100 dark:bg-gray-700/40 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60 focus:outline-none focus:ring-2 focus:ring-gray-400"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:close",
                    class: "text-base"
                  }, null, _parent3, _scopeId2));
                  _push3(`</button></div><div class="mb-4 flex flex-wrap gap-1.5"${_scopeId2}><!--[-->`);
                  ssrRenderList(unref(enabledLocales), (loc) => {
                    _push3(`<button type="button" class="${ssrRenderClass([
                      manageLocale.value === loc.code ? "bg-blue-600 text-white" : "bg-gray-100 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700",
                      "inline-flex items-center gap-1 h-8 px-2.5 rounded-full text-[11px] font-medium uppercase tracking-wide transition"
                    ])}"${_scopeId2}>${ssrInterpolate(loc.code)}</button>`);
                  });
                  _push3(`<!--]--></div><p class="text-[11px] text-gray-500 dark:text-gray-400 mb-3"${_scopeId2}>${ssrInterpolate(unref(t)("categories.manageLocaleHint"))}</p><div class="flex flex-col sm:flex-row gap-2 mb-4"${_scopeId2}><div class="flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus-within:ring-2 focus-within:ring-green-500 transition"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:format-list-bulleted",
                    class: "ms-3 text-gray-500 dark:text-gray-400 text-base"
                  }, null, _parent3, _scopeId2));
                  _push3(`<input${ssrRenderAttr("value", newName.value)}${ssrRenderAttr("placeholder", unref(t)("categories.newName"))} class="flex-1 h-full bg-transparent px-2 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none"${_scopeId2}></div><button type="button" class="w-full sm:w-auto inline-flex items-center gap-1 h-9 px-4 rounded-md text-sm font-medium bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 hover:bg-green-200 dark:hover:bg-green-900/60 focus:outline-none focus:ring-2 focus:ring-green-500"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:plus",
                    class: "text-base"
                  }, null, _parent3, _scopeId2));
                  ...
                  _push3(`<input${ssrRenderAttr("value", searchTerm.value)}${ssrRenderAttr("placeholder", unref(t)("categories.searchPlaceholder"))} class="flex-1 h-full bg-transparent px-2 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none"${_scopeId2}></div></div><div class="divide-y divide-gray-200 dark:divide-gray-700 max-h-[50vh] overflow-y-auto"${_scopeId2}>`);
                  if (!filteredGroups.value.length) {
                    _push3(`<div class="py-8 text-center text-sm text-gray-500 dark:text-gray-400"${_scopeId2}>${ssrInterpolate(unref(t)("categories.empty"))}</div>`);
                  } else {
                    _push3(`<!---->`);
                  }
                  _push3(`<!--[-->`);
                  ssrRenderList(filteredGroups.value, (group) => {
                    var _a;
                    _push3(`<div class="py-3 space-y-2"${_scopeId2}><div class="flex items-start justify-between gap-2"${_scopeId2}><div class="min-w-0 flex-1"${_scopeId2}>`);
                    if (editingId.value && group.rows.some((r) => r.id === editingId.value)) {
                      _push3(`<input${ssrRenderAttr("value", editingName.value)} class="w-full px-2 py-1 border rounded dark:bg-gray-600 dark:border-gray-500 dark:text-white text-sm"${_scopeId2}>`);
                    } else {
                      _push3(`<!--[--><span class="text-sm font-medium text-gray-900 dark:text-gray-100"${_scopeId2}>${ssrInterpolate(groupDisplayName(group))}</span><span class="ms-2 text-[11px] text-gray-400"${_scopeId2}>/${ssrInterpolate(group.slug)}</span><!--]-->`);
                    }
                    _push3(`</div>`);
                    if (editingId.value && group.rows.some((r) => r.id === editingId.value)) {
                      _push3(`<div class="flex gap-1 shrink-0"${_scopeId2}><button type="button" class="inline-flex items-center gap-1 h-8 px-2 rounded-md text-xs font-medium bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300"${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), { icon: "mdi:check" }, null, _parent3, _scopeId2));
                      _push3(` ${ssrInterpolate(unref(t)("common.save"))}</button><button type="button" class="inline-flex items-center gap-1 h-8 px-2 rounded-md text-xs font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300"${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), { icon: "mdi:close" }, null, _parent3, _scopeId2));
                      _push3(` ${ssrInterpolate(unref(t)("common.cancel"))}</button></div>`);
                    } else {
                      _push3(`<div class="flex gap-1 shrink-0"${_scopeId2}>`);
                      if (group.rows.find((r) => r.locale === manageLocale.value)) {
                        _push3(`<button type="button" class="inline-flex items-center justify-center h-8 w-8 rounded-md text-amber-600 dark:text-amber-300 hover:bg-amber-50 dark:hover:bg-amber-900/30"${ssrRenderAttr("title", unref(t)("common.edit"))}${_scopeId2}>`);
                        _push3(ssrRenderComponent(unref(Icon), {
                          icon: "mdi:pencil-outline",
                          class: "text-base"
                        }, null, _parent3, _scopeId2));
                      ...
                      if (group.rows.find((r) => r.locale === manageLocale.value)) {
                        _push3(`<button type="button" class="inline-flex items-center justify-center h-8 w-8 rounded-md text-red-600 dark:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/30"${ssrRenderAttr("title", unref(t)("categories.deleteLocale"))}${_scopeId2}>`);
                        _push3(ssrRenderComponent(unref(Icon), {
                          icon: "mdi:delete-outline",
                          class: "text-base"
                        }, null, _parent3, _scopeId2));
                      ...
                      _push3(`<button type="button" class="inline-flex items-center justify-center h-8 w-8 rounded-md text-red-700 dark:text-red-200 hover:bg-red-100 dark:hover:bg-red-900/40"${ssrRenderAttr("title", unref(t)("categories.deleteAllLanguages"))}${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), {
                    ...
                    _push3(`</div><div class="flex flex-wrap items-center gap-1.5"${_scopeId2}><!--[-->`);
                    ssrRenderList(unref(enabledLocales), (loc) => {
                      _push3(`<button type="button" class="${ssrRenderClass([localeChipClass(group, loc.code), "inline-flex items-center gap-1 h-7 px-2 rounded-full text-[10px] font-semibold uppercase tracking-wide transition"])}"${ssrIncludeBooleanAttr(translating.value) ? " disabled" : ""}${ssrRenderAttr("title", localeChipTitle(group, loc.code))}${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), {
                        icon: hasLocale(group, loc.code) ? "mdi:check" : "mdi:plus",
                        class: "text-sm"
                      }, null, _parent3, _scopeId2));
                      _push3(` ${ssrInterpolate(loc.code)}</button>`);
                    });
                    _push3(`<!--]--></div>`);
                    if (((_a = translateFor.value) == null ? void 0 : _a.groupKey) === group.key) {
                      _push3(`<div class="flex flex-col sm:flex-row gap-2 pt-1"${_scopeId2}><input${ssrRenderAttr("value", translateName.value)} type="text" class="flex-1 h-8 rounded-md px-2 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"${ssrRenderAttr("placeholder", unref(t)("categories.translationName", { locale: translateFor.value.code }))}${_scopeId2}><div class="flex gap-1"${_scopeId2}><button type="button" class="h-8 px-3 rounded-md text-xs font-medium bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300"${ssrIncludeBooleanAttr(translating.value || !translateName.value.trim()) ? " disabled" : ""}${_scopeId2}>${ssrInterpolate(unref(t)("categories.addTranslation"))}</button><button type="button" class="h-8 px-3 rounded-md text-xs font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300"${_scopeId2}>${ssrInterpolate(unref(t)("common.cancel"))}</button></div></div>`);
                    } else {
                      _push3(`<!---->`);
                    }
                    _push3(`</div>`);
                  });
                  _push3(`<!--]--></div><div class="mt-4 text-end"${_scopeId2}><button type="button" class="inline-flex items-center gap-1 h-9 px-4 rounded-md text-sm font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60 focus:outline-none focus:ring-2 focus:ring-gray-400"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:close",
                    class: "text-base"
                  }, null, _parent3, _scopeId2));
                  ...
                  return [
                    ...
                    createVNode("div", { class: "mb-4 flex flex-wrap gap-1.5" }, [
                      (openBlock(true), createBlock(Fragment, null, renderList(unref(enabledLocales), (loc) => {
                        return openBlock(), createBlock("button", {
                          ...
                          onClick: ($event) => manageLocale.value = loc.code
                        }, toDisplayString(loc.code), 11, ["onClick"]);
                      }), 128))
                    ]),
                    createVNode("p", { class: "text-[11px] text-gray-500 dark:text-gray-400 mb-3" }, toDisplayString(unref(t)("categories.manageLocaleHint")), 1),
                    createVNode("div", { class: "flex flex-col sm:flex-row gap-2 mb-4" }, [
                      createVNode("div", { class: "flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus-within:ring-2 focus-within:ring-green-500 transition" }, [
                        ...
                        withDirectives(createVNode("input", {
                          ...
                          onKeydown: withKeys(withModifiers(addCategory, ["prevent"]), ["enter"]),
                        ...
                        }, null, 40, ["onUpdate:modelValue", "onKeydown", "placeholder"]), [
                    ...
                    createVNode("div", { class: "divide-y divide-gray-200 dark:divide-gray-700 max-h-[50vh] overflow-y-auto" }, [
                      !filteredGroups.value.length ? (openBlock(), createBlock("div", {
                        key: 0,
                        class: "py-8 text-center text-sm text-gray-500 dark:text-gray-400"
                      }, toDisplayString(unref(t)("categories.empty")), 1)) : createCommentVNode("", true),
                        ...
                        return openBlock(), createBlock("div", {
                          ...
                          createVNode("div", { class: "flex items-start justify-between gap-2" }, [
                            createVNode("div", { class: "min-w-0 flex-1" }, [
                              editingId.value && group.rows.some((r) => r.id === editingId.value) ? withDirectives((openBlock(), createBlock("input", {
                                key: 0,
                                "onUpdate:modelValue": ($event) => editingName.value = $event,
                                onKeydown: withKeys(withModifiers(($event) => updateCategory(editingId.value), ["prevent"]), ["enter"]),
                                ...
                                createVNode("span", { class: "text-sm font-medium text-gray-900 dark:text-gray-100" }, toDisplayString(groupDisplayName(group)), 1),
                                createVNode("span", { class: "ms-2 text-[11px] text-gray-400" }, "/" + toDisplayString(group.slug), 1)
                              ], 64))
                            ]),
                            editingId.value && group.rows.some((r) => r.id === editingId.value) ? (openBlock(), createBlock("div", {
                              key: 0,
                              class: "flex gap-1 shrink-0"
                            }, [
                              createVNode("button", {
                                type: "button",
                                onClick: ($event) => updateCategory(editingId.value),
                                ...
                                createVNode(unref(Icon), { icon: "mdi:check" }),
                              ...
                              createVNode("button", {
                                ...
                                createVNode(unref(Icon), { icon: "mdi:close" }),
                              ...
                              group.rows.find((r) => r.locale === manageLocale.value) ? (openBlock(), createBlock("button", {
                                key: 0,
                                type: "button",
                                onClick: ($event) => startEdit(group.rows.find((r) => r.locale === manageLocale.value)),
                              ...
                              group.rows.find((r) => r.locale === manageLocale.value) ? (openBlock(), createBlock("button", {
                                key: 1,
                                type: "button",
                                onClick: ($event) => askDeleteLocale(group.rows.find((r) => r.locale === manageLocale.value)),
                                ...
                                title: unref(t)("categories.deleteLocale")
                              }, [
                                createVNode(unref(Icon), {
                                  icon: "mdi:delete-outline",
                                  class: "text-base"
                              ...
                              createVNode("button", {
                                type: "button",
                                onClick: ($event) => askDeleteAllLanguages(group),
                                ...
                                title: unref(t)("categories.deleteAllLanguages")
                              }, [
                                createVNode(unref(Icon), {
                                  icon: "mdi:delete-sweep-outline",
                                  class: "text-base"
                          ...
                          createVNode("div", { class: "flex flex-wrap items-center gap-1.5" }, [
                            (openBlock(true), createBlock(Fragment, null, renderList(unref(enabledLocales), (loc) => {
                              return openBlock(), createBlock("button", {
                                ...
                                type: "button",
                                class: ["inline-flex items-center gap-1 h-7 px-2 rounded-full text-[10px] font-semibold uppercase tracking-wide transition", localeChipClass(group, loc.code)],
                                disabled: translating.value,
                                title: localeChipTitle(group, loc.code),
                                onClick: ($event) => onLocaleChip(group, loc.code)
                              }, [
                                createVNode(unref(Icon), {
                                  icon: hasLocale(group, loc.code) ? "mdi:check" : "mdi:plus",
                                  class: "text-sm"
                                }, null, 8, ["icon"]),
                                createTextVNode(" " + toDisplayString(loc.code), 1)
                              ], 10, ["disabled", "title", "onClick"]);
                            }), 128))
                          ]),
                          ((_a = translateFor.value) == null ? void 0 : _a.groupKey) === group.key ? (openBlock(), createBlock("div", {
                            key: 0,
                            class: "flex flex-col sm:flex-row gap-2 pt-1"
                          }, [
                            withDirectives(createVNode("input", {
                              "onUpdate:modelValue": ($event) => translateName.value = $event,
                              type: "text",
                              ...
                              placeholder: unref(t)("categories.translationName", { locale: translateFor.value.code }),
                              onKeydown: withKeys(withModifiers(saveTranslation, ["prevent"]), ["enter"])
                            }, null, 40, ["onUpdate:modelValue", "placeholder", "onKeydown"]), [
                              [vModelText, translateName.value]
                            ]),
                            createVNode("div", { class: "flex gap-1" }, [
                              createVNode("button", {
                                ...
                                disabled: translating.value || !translateName.value.trim(),
                                onClick: saveTranslation
                              }, toDisplayString(unref(t)("categories.addTranslation")), 9, ["disabled"]),
                              createVNode("button", {
                                type: "button",
            ...
            return [
              ...
              createVNode("div", { class: "fixed inset-0 flex items-center justify-center p-4" }, [
                createVNode(unref(DialogPanel), { class: "w-full max-w-lg overflow-auto rounded-2xl bg-white dark:bg-gray-800 p-4 sm:p-6 text-start shadow-xl max-h-[90vh]" }, {
                  default: withCtx(() => [
                    createVNode("div", { class: "flex items-center justify-between mb-4" }, [
                      createVNode(unref(DialogTitle), { class: "text-lg font-semibold text-gray-900 dark:text-gray-100" }, {
                        default: withCtx(() => [
      ...
      _push(ssrRenderComponent(_sfc_main$g, {
        open: !!deleteLocaleRow.value,
        title: unref(t)("categories.deleteLocaleTitle"),
        description: deleteLocaleRow.value ? unref(t)("categories.deleteLocaleDescription", {
          name: deleteLocaleRow.value.name,
          locale: deleteLocaleRow.value.locale
        }) : "",
        body: unref(t)("categories.deleteLocaleBody"),
        "confirm-label": unref(t)("categories.deleteLocale"),
        onConfirm: confirmDeleteLocale,
        onCancel: ($event) => deleteLocaleRow.value = null
      }, null, _parent));
      _push(ssrRenderComponent(_sfc_main$g, {
        open: !!deleteAllGroup.value,
        title: unref(t)("categories.deleteAllTitle"),
        description: deleteAllGroup.value ? unref(t)("categories.deleteAllDescription", {
          name: groupDisplayName(deleteAllGroup.value)
        }) : "",
        body: unref(t)("categories.deleteAllBody"),
        "confirm-label": unref(t)("categories.deleteAllLanguages"),
        onConfirm: confirmDeleteAllLanguages,
        onCancel: ($event) => deleteAllGroup.value = null
      }, null, _parent));
      _push(`</div>`);
    };
  }
...
const _sfc_main$a = {
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    function localeChipClass(group, code) {
      if (hasLocale(group, code)) {
        return code === manageLocale.value ? "bg-indigo-600 text-white" : "bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300";
      }
      return "bg-gray-100 text-gray-500 dark:bg-gray-700/40 dark:text-gray-400 hover:bg-green-50 hover:text-green-700 dark:hover:bg-green-900/30 dark:hover:text-green-300 border border-dashed border-gray-300 dark:border-gray-600";
    }
    function localeChipTitle(group, code) {
    ...
    async function saveTranslation() {
      ...
      try {
        ...
        if (!first.translation_group_id) {
          groupId = crypto.randomUUID();
          await supabase.from("series").update({ translation_group_id: groupId }).eq("id", first.id);
        }
        const slug = group.slug || slugify(name) || Date.now().toString();
        ...
        toast.error((e == null ? void 0 : e.message) || t("series.translationFailed"));
      } finally {
        translating.value = false;
      }
    }
    async function load() {
      var _a;
      error.value = "";
      const { data, error: err } = await supabase.from("series").select("id, name, slug, locale, translation_group_id").order("name", { ascending: true });
      if (err) {
    ...
    function slugify(raw) {
      return String(raw || "").toLowerCase().trim().replace(/[^a-z0-9\s-]/g, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-+/, "").replace(/-+$/, "").slice(0, 80);
    }
    async function addSeries() {
      const name = newName.value.trim();
      if (!name) return;
      const loc = manageLocale.value || "en";
      if (series.value.some(
    ...
    async function updateSeries(id) {
      ...
      if (series.value.some(
        (s) => s.id !== id && s.locale === (existing == null ? void 0 : existing.locale) && s.name.toLowerCase() === name.toLowerCase()
      )) {
        toast.error(t("series.exists"));
        return;
      }
      const { error: err } = await supabase.from("series").update({ name }).eq("id", id);
      if (err) {
        toast.error(err.message);
        return;
      }
    ...
    async function confirmDeleteAllLanguages() {
      ...
      try {
        ...
        deleteAllGroup.value = null;
        if (((_a = translateFor.value) == null ? void 0 : _a.groupKey) === group.key) cancelTranslation();
        cancelEdit();
        await load();
        emit("changed");
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps(_ctx.$attrs, _attrs))}><button type="button" class="inline-flex items-center justify-center gap-2 h-9 px-4 rounded-md text-sm font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-200 dark:hover:bg-indigo-900/60 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-auto">`);
      _push(ssrRenderComponent(unref(Icon), {
        icon: "mdi:bookshelf",
        class: "text-base"
      }, null, _parent));
            ...
            _push2(ssrRenderComponent(unref(DialogPanel), { class: "w-full max-w-lg overflow-auto rounded-2xl bg-white dark:bg-gray-800 p-4 sm:p-6 text-start shadow-xl max-h-[90vh]" }, {
              default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                if (_push3) {
                  ...
                  ssrRenderList(unref(enabledLocales), (loc) => {
                    _push3(`<button type="button" class="${ssrRenderClass([
                      manageLocale.value === loc.code ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700",
                      "inline-flex items-center gap-1 h-8 px-2.5 rounded-full text-[11px] font-medium uppercase tracking-wide transition"
                    ])}"${_scopeId2}>${ssrInterpolate(loc.code)}</button>`);
                  });
                  _push3(`<!--]--></div><p class="text-[11px] text-gray-500 dark:text-gray-400 mb-3"${_scopeId2}>${ssrInterpolate(unref(t)("series.manageLocaleHint"))}</p><div class="flex flex-col sm:flex-row gap-2 mb-4"${_scopeId2}><div class="flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus-within:ring-2 focus-within:ring-green-500 transition"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:bookshelf",
                    class: "ms-3 text-gray-500 dark:text-gray-400 text-base"
                  }, null, _parent3, _scopeId2));
                  _push3(`<input${ssrRenderAttr("value", newName.value)}${ssrRenderAttr("placeholder", unref(t)("series.newName"))} class="flex-1 h-full bg-transparent px-2 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none"${_scopeId2}></div><button type="button" class="w-full sm:w-auto inline-flex items-center gap-1 h-9 px-4 rounded-md text-sm font-medium bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 hover:bg-green-200 dark:hover:bg-green-900/60 focus:outline-none focus:ring-2 focus:ring-green-500"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:plus",
                    class: "text-base"
                  }, null, _parent3, _scopeId2));
                  _push3(`<span${_scopeId2}>${ssrInterpolate(unref(t)("series.add"))}</span></button></div><div class="mb-4 relative"${_scopeId2}><div class="flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 transition"${_scopeId2}>`);
                  _push3(ssrRenderComponent(unref(Icon), {
                    icon: "mdi:magnify",
                    class: "ms-3 text-gray-500 dark:text-gray-400 text-base"
                  }, null, _parent3, _scopeId2));
                  _push3(`<input${ssrRenderAttr("value", searchTerm.value)}${ssrRenderAttr("placeholder", unref(t)("series.searchPlaceholder"))} class="flex-1 h-full bg-transparent px-2 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none"${_scopeId2}></div></div><div class="divide-y divide-gray-200 dark:divide-gray-700 max-h-[50vh] overflow-y-auto"${_scopeId2}>`);
                  if (!filteredGroups.value.length) {
                    _push3(`<div class="py-8 text-center text-sm text-gray-500 dark:text-gray-400"${_scopeId2}>${ssrInterpolate(error.value || unref(t)("series.empty"))}</div>`);
                  } else {
                    _push3(`<!---->`);
                  }
                  _push3(`<!--[-->`);
                  ssrRenderList(filteredGroups.value, (group) => {
                    ...
                    _push3(`</div>`);
                    if (editingId.value && group.rows.some((r) => r.id === editingId.value)) {
                      _push3(`<div class="flex gap-1 shrink-0"${_scopeId2}><button type="button" class="inline-flex items-center gap-1 h-8 px-2 rounded-md text-xs font-medium bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300"${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), { icon: "mdi:check" }, null, _parent3, _scopeId2));
                      _push3(` ${ssrInterpolate(unref(t)("common.save"))}</button><button type="button" class="inline-flex items-center gap-1 h-8 px-2 rounded-md text-xs font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300"${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), { icon: "mdi:close" }, null, _parent3, _scopeId2));
                      _push3(` ${ssrInterpolate(unref(t)("common.cancel"))}</button></div>`);
                      ...
                      if (group.rows.find((r) => r.locale === manageLocale.value)) {
                        _push3(`<button type="button" class="inline-flex items-center justify-center h-8 w-8 rounded-md text-red-600 dark:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/30"${ssrRenderAttr("title", unref(t)("series.deleteLocale"))}${_scopeId2}>`);
                        _push3(ssrRenderComponent(unref(Icon), {
                          icon: "mdi:delete-outline",
                          class: "text-base"
                        }, null, _parent3, _scopeId2));
                      ...
                      _push3(`<button type="button" class="inline-flex items-center justify-center h-8 w-8 rounded-md text-red-700 dark:text-red-200 hover:bg-red-100 dark:hover:bg-red-900/40"${ssrRenderAttr("title", unref(t)("series.deleteAllLanguages"))}${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), {
                        icon: "mdi:delete-sweep-outline",
                        class: "text-base"
                      }, null, _parent3, _scopeId2));
                    ...
                    _push3(`<!--]--></div>`);
                    if (((_a = translateFor.value) == null ? void 0 : _a.groupKey) === group.key) {
                      _push3(`<div class="flex flex-col sm:flex-row gap-2 pt-1"${_scopeId2}><input${ssrRenderAttr("value", translateName.value)} type="text" class="flex-1 h-8 rounded-md px-2 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"${ssrRenderAttr(
                        "placeholder",
                        unref(t)("series.translationName", { locale: translateFor.value.code })
                      )}${_scopeId2}><div class="flex gap-1"${_scopeId2}><button type="button" class="h-8 px-3 rounded-md text-xs font-medium bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300"${ssrIncludeBooleanAttr(translating.value || !translateName.value.trim()) ? " disabled" : ""}${_scopeId2}>${ssrInterpolate(unref(t)("series.addTranslation"))}</button><button type="button" class="h-8 px-3 rounded-md text-xs font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300"${_scopeId2}>${ssrInterpolate(unref(t)("common.cancel"))}</button></div></div>`);
                    } else {
                      _push3(`<!---->`);
                    }
                    _push3(`</div>`);
                  ...
                  return [
                    ...
                    createVNode("p", { class: "text-[11px] text-gray-500 dark:text-gray-400 mb-3" }, toDisplayString(unref(t)("series.manageLocaleHint")), 1),
                    createVNode("div", { class: "flex flex-col sm:flex-row gap-2 mb-4" }, [
                      createVNode("div", { class: "flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus-within:ring-2 focus-within:ring-green-500 transition" }, [
                        createVNode(unref(Icon), {
                          icon: "mdi:bookshelf",
                        ...
                        withDirectives(createVNode("input", {
                          "onUpdate:modelValue": ($event) => newName.value = $event,
                          onKeydown: withKeys(withModifiers(addSeries, ["prevent"]), ["enter"]),
                      ...
                      createVNode("button", {
                        ...
                        createVNode("span", null, toDisplayString(unref(t)("series.add")), 1)
                      ])
                    ]),
                    createVNode("div", { class: "mb-4 relative" }, [
                      createVNode("div", { class: "flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 transition" }, [
                        createVNode(unref(Icon), {
                          icon: "mdi:magnify",
                          class: "ms-3 text-gray-500 dark:text-gray-400 text-base"
                        }),
                        withDirectives(createVNode("input", {
                          "onUpdate:modelValue": ($event) => searchTerm.value = $event,
                          placeholder: unref(t)("series.searchPlaceholder"),
                          class: "flex-1 h-full bg-transparent px-2 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:outline-none"
                        }, null, 8, ["onUpdate:modelValue", "placeholder"]), [
                          [vModelText, searchTerm.value]
                        ])
                    ...
                    createVNode("div", { class: "divide-y divide-gray-200 dark:divide-gray-700 max-h-[50vh] overflow-y-auto" }, [
                      !filteredGroups.value.length ? (openBlock(), createBlock("div", {
                        key: 0,
                        class: "py-8 text-center text-sm text-gray-500 dark:text-gray-400"
                      }, toDisplayString(error.value || unref(t)("series.empty")), 1)) : createCommentVNode("", true),
                      (openBlock(true), createBlock(Fragment, null, renderList(filteredGroups.value, (group) => {
                        var _a;
                        return openBlock(), createBlock("div", {
                          key: group.key,
                          ...
                          createVNode("div", { class: "flex items-start justify-between gap-2" }, [
                            createVNode("div", { class: "min-w-0 flex-1" }, [
                              editingId.value && group.rows.some((r) => r.id === editingId.value) ? withDirectives((openBlock(), createBlock("input", {
                                key: 0,
                                "onUpdate:modelValue": ($event) => editingName.value = $event,
                                onKeydown: withKeys(withModifiers(($event) => updateSeries(editingId.value), ["prevent"]), ["enter"]),
                                class: "w-full px-2 py-1 border rounded dark:bg-gray-600 dark:border-gray-500 dark:text-white text-sm"
                              }, null, 40, ["onUpdate:modelValue", "onKeydown"])), [
                                [vModelText, editingName.value]
                              ]) : (openBlock(), createBlock(Fragment, { key: 1 }, [
                            ...
                            editingId.value && group.rows.some((r) => r.id === editingId.value) ? (openBlock(), createBlock("div", {
                              ...
                              createVNode("button", {
                                type: "button",
                                onClick: ($event) => updateSeries(editingId.value),
                              ...
                              group.rows.find((r) => r.locale === manageLocale.value) ? (openBlock(), createBlock("button", {
                                key: 1,
                                type: "button",
                                onClick: ($event) => askDeleteLocale(
                                  group.rows.find((r) => r.locale === manageLocale.value)
                                ),
                                class: "inline-flex items-center justify-center h-8 w-8 rounded-md text-red-600 dark:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/30",
                                title: unref(t)("series.deleteLocale")
                              }, [
                                createVNode(unref(Icon), {
                                  icon: "mdi:delete-outline",
                                  class: "text-base"
                              ...
                              createVNode("button", {
                                ...
                                onClick: ($event) => askDeleteAllLanguages(group),
                                class: "inline-flex items-center justify-center h-8 w-8 rounded-md text-red-700 dark:text-red-200 hover:bg-red-100 dark:hover:bg-red-900/40",
                                title: unref(t)("series.deleteAllLanguages")
                              }, [
                                createVNode(unref(Icon), {
                                  icon: "mdi:delete-sweep-outline",
                                  class: "text-base"
                          ...
                          ((_a = translateFor.value) == null ? void 0 : _a.groupKey) === group.key ? (openBlock(), createBlock("div", {
                            ...
                            withDirectives(createVNode("input", {
                              ...
                              type: "text",
                              class: "flex-1 h-8 rounded-md px-2 text-sm border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100",
                              placeholder: unref(t)("series.translationName", { locale: translateFor.value.code }),
                              onKeydown: withKeys(withModifiers(saveTranslation, ["prevent"]), ["enter"])
                            }, null, 40, ["onUpdate:modelValue", "placeholder", "onKeydown"]), [
                              [vModelText, translateName.value]
                            ]),
                            createVNode("div", { class: "flex gap-1" }, [
                              createVNode("button", {
                                ...
                                disabled: translating.value || !translateName.value.trim(),
                                onClick: saveTranslation
                              }, toDisplayString(unref(t)("series.addTranslation")), 9, ["disabled"]),
                              createVNode("button", {
                                type: "button",
                                class: "h-8 px-3 rounded-md text-xs font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300",
                                onClick: cancelTranslation
      ...
      _push(ssrRenderComponent(_sfc_main$g, {
        open: !!deleteLocaleRow.value,
        title: unref(t)("series.deleteLocaleTitle"),
        description: deleteLocaleRow.value ? unref(t)("series.deleteLocaleDescription", {
          name: deleteLocaleRow.value.name,
          locale: deleteLocaleRow.value.locale
        }) : "",
        body: unref(t)("series.deleteLocaleBody"),
        "confirm-label": unref(t)("series.deleteLocale"),
        onConfirm: confirmDeleteLocale,
        onCancel: ($event) => deleteLocaleRow.value = null
      }, null, _parent));
      _push(ssrRenderComponent(_sfc_main$g, {
        open: !!deleteAllGroup.value,
        title: unref(t)("series.deleteAllTitle"),
        description: deleteAllGroup.value ? unref(t)("series.deleteAllDescription", {
          name: groupDisplayName(deleteAllGroup.value)
        }) : "",
        body: unref(t)("series.deleteAllBody"),
        "confirm-label": unref(t)("series.deleteAllLanguages"),
        onConfirm: confirmDeleteAllLanguages,
        onCancel: ($event) => deleteAllGroup.value = null
      }, null, _parent));
      _push(`</div>`);
...
const _sfc_main$6 = {
  ...
  setup(__props) {
    ...
    function hasMetaLocale(code) {
      ...
      const tr = (_a = branding.metaTranslations.value) == null ? void 0 : _a[code];
      return !!((tr == null ? void 0 : tr.siteName) || (tr == null ? void 0 : tr.siteDescription));
    }
    function loadLocaleFields(code) {
      ...
      const tr = (_a = branding.metaTranslations.value) == null ? void 0 : _a[code];
      if ((tr == null ? void 0 : tr.siteName) || (tr == null ? void 0 : tr.siteDescription)) {
    ...
    watch(
      () => {
        var _a;
        return [branding.primaryLocale.value, (_a = branding.enabledLocales.value) == null ? void 0 : _a.join(",")];
      },
      () => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm p-6 space-y-6 min-w-0 w-full" }, _attrs))} data-v-93543a46><div class="flex flex-wrap items-start gap-3 sm:flex-nowrap" data-v-93543a46><div class="p-2 rounded-lg bg-pink-50 dark:bg-pink-900/30 text-pink-600 dark:text-pink-300" data-v-93543a46>`);
      _push(ssrRenderComponent(unref(Icon), {
        icon: "mdi:brush",
        class: "w-6 h-6"
      }, null, _parent));
      _push(`</div><div class="flex-1 min-w-0" data-v-93543a46><h2 class="text-sm font-semibold tracking-wide text-gray-800 dark:text-gray-100 uppercase break-words" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.title"))}</h2><p class="text-[12px] text-gray-500 dark:text-gray-400 mt-1 break-words" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.description"))}</p></div></div><form class="space-y-5" data-v-93543a46>`);
      if (enabledLocalesList.value.length > 1) {
        _push(`<div class="space-y-2" data-v-93543a46><div class="flex flex-wrap gap-1.5" data-v-93543a46><!--[-->`);
        ssrRenderList(enabledLocalesList.value, (code) => {
          _push(`<button type="button" class="${ssrRenderClass([
            manageLocale.value === code ? "bg-blue-600 text-white" : hasMetaLocale(code) ? "bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/50" : "bg-gray-100 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 border border-dashed border-gray-300 dark:border-gray-600",
            "inline-flex items-center gap-1 h-8 px-2.5 rounded-full text-[11px] font-medium uppercase tracking-wide transition"
          ])}"${ssrRenderAttr(
            "title",
            hasMetaLocale(code) ? unref(t)("settings.siteMeta.editLocale", { locale: code }) : unref(t)("settings.siteMeta.addTranslationFor", { locale: code })
          )} data-v-93543a46>`);
          _push(ssrRenderComponent(unref(Icon), {
            icon: hasMetaLocale(code) ? "mdi:check" : "mdi:plus",
            class: "text-sm"
          }, null, _parent));
          _push(` ${ssrInterpolate(code)} `);
          if (code === primaryLocaleCode.value) {
            _push(`<span class="text-[9px] font-semibold opacity-80 normal-case tracking-normal" data-v-93543a46>(${ssrInterpolate(unref(t)("settings.siteMeta.primary"))})</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</button>`);
        });
        _push(`<!--]--></div><p class="text-[11px] text-gray-500 dark:text-gray-400" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.manageLocaleHint"))}</p></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div data-v-93543a46><label class="block text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-400 mb-1" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.siteName"))} `);
      if (enabledLocalesList.value.length > 1) {
        _push(`<span class="font-normal normal-case tracking-normal text-gray-400" data-v-93543a46>(${ssrInterpolate(manageLocale.value)})</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</label><input${ssrRenderAttr("value", siteName.value)} type="text" class="w-full h-11 rounded-md px-3 bg-white dark:bg-gray-900/40 border border-gray-300 dark:border-gray-600 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.siteNamePlaceholder"))}${ssrIncludeBooleanAttr(manageLocale.value === primaryLocaleCode.value) ? " required" : ""} data-v-93543a46></div><div data-v-93543a46><label class="block text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-400 mb-1" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.siteDescription"))} `);
      if (enabledLocalesList.value.length > 1) {
        _push(`<span class="font-normal normal-case tracking-normal text-gray-400" data-v-93543a46>(${ssrInterpolate(manageLocale.value)})</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</label><textarea rows="3" class="w-full rounded-md px-3 py-2 bg-white dark:bg-gray-900/40 border border-gray-300 dark:border-gray-600 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.siteDescriptionPlaceholder"))} data-v-93543a46>${ssrInterpolate(siteDescription.value)}</textarea></div>`);
      if (manageLocale.value !== primaryLocaleCode.value && hasMetaLocale(manageLocale.value)) {
        _push(`<div class="flex justify-end" data-v-93543a46><button type="button" class="inline-flex items-center gap-1 h-8 px-3 rounded-md text-[11px] font-medium bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-900/60"${ssrIncludeBooleanAttr(saving.value) ? " disabled" : ""} data-v-93543a46>`);
        _push(ssrRenderComponent(unref(Icon), {
          icon: "mdi:delete-outline",
          class: "text-sm"
        }, null, _parent));
        _push(` ${ssrInterpolate(unref(t)("settings.siteMeta.deleteTranslation"))}</button></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div data-v-93543a46><label class="block text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-400 mb-1" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.twitterHandle"))}</label><div class="flex items-center h-11 rounded-md bg-white dark:bg-gray-900/40 border border-gray-300 dark:border-gray-600 focus-within:ring-2 focus-within:ring-blue-500 overflow-hidden" data-v-93543a46><span class="ps-3 text-sm text-gray-400 select-none" data-v-93543a46>@</span><input${ssrRenderAttr("value", twitterHandle.value)} type="text" class="flex-1 h-full px-2 bg-transparent text-sm focus:outline-none"${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.twitterHandlePlaceholder"))} autocomplete="off" maxlength="15" data-v-93543a46></div><p class="mt-1 text-[11px] text-gray-500 dark:text-gray-400" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.twitterHint"))}</p></div><div class="space-y-3" data-v-93543a46><div class="flex flex-wrap items-center justify-between gap-2" data-v-93543a46><label class="text-xs font-semibold uppercase tracking-wide text-gray-600 dark:text-gray-400" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.socialLinks"))} `);
      if (links.value.length) {
        _push(`<span class="font-normal text-gray-400" data-v-93543a46>(${ssrInterpolate(links.value.length)})</span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</label><div class="flex flex-wrap items-center gap-2 justify-end" data-v-93543a46>`);
      if (links.value.length) {
        _push(`<button type="button" class="inline-flex items-center gap-1 h-7 px-2 rounded-md text-[10px] font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700/60" data-v-93543a46>`);
        _push(ssrRenderComponent(unref(Icon), {
      ...
      _push(`<button type="button" class="inline-flex items-center gap-1 h-7 px-3 rounded-md text-[11px] font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700/60" data-v-93543a46>`);
      _push(ssrRenderComponent(unref(Icon), {
        icon: "mdi:plus",
        class: "text-sm"
      }, null, _parent));
      ...
      if (links.value.length === 0) {
        _push(`<div class="text-[11px] text-gray-500 dark:text-gray-400" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.noLinks"))}</div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="max-h-64 overflow-auto pe-1 custom-scroll space-y-3" data-social-links style="${ssrRenderStyle(!collapsed.value ? null : { display: "none" })}" data-v-93543a46><ul class="space-y-3" data-v-93543a46><!--[-->`);
      ssrRenderList(links.value, (link, i) => {
        _push(`<li class="group relative border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900/30 flex flex-col gap-2" data-v-93543a46><div class="grid grid-cols-1 sm:grid-cols-5 gap-2 items-center" data-v-93543a46><input${ssrRenderAttr("value", link.label)} type="text" data-link-label${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.labelPlaceholder"))} class="h-9 rounded-md px-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 sm:col-span-2" data-v-93543a46><input${ssrRenderAttr("value", link.url)} type="text"${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.urlPlaceholder"))} class="h-9 rounded-md px-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500 sm:col-span-3" data-v-93543a46></div><div class="flex flex-wrap items-center gap-2 relative w-full" data-v-93543a46><button type="button" class="w-8 h-8 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 flex items-center justify-center text-gray-600 dark:text-gray-300 shrink-0 hover:ring-2 hover:ring-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500"${ssrRenderAttr("aria-expanded", openIconIndex.value === i)}${ssrRenderAttr("aria-label", unref(t)("settings.siteMeta.pickIcon"))} data-v-93543a46>`);
        _push(ssrRenderComponent(unref(Icon), {
        ...
        ssrRenderTeleport(_push, (_push2) => {
          if (openIconIndex.value === i) {
            _push2(`<div class="icon-picker-panel fixed z-[1000] w-72 p-3 rounded-lg bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 shadow-lg space-y-2" style="${ssrRenderStyle(pickerStyle.value)}" data-v-93543a46><div class="flex items-center gap-2" data-v-93543a46><input${ssrRenderAttr("value", iconSearch.value)} type="text"${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.iconSearchPlaceholder"))} class="icon-picker-search flex-1 h-8 px-2 rounded-md bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-[11px] focus:outline-none focus:ring-2 focus:ring-blue-500" data-v-93543a46><button type="button" class="h-8 px-2 rounded-md text-[11px] font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60"${ssrRenderAttr("aria-label", unref(t)("common.close"))} data-v-93543a46> \u2715 </button></div><div class="max-h-56 overflow-auto space-y-3 pe-1 custom-scroll" data-v-93543a46>`);
            if (filteredGroups.value.length) {
              _push2(`<!--[-->`);
              ssrRenderList(filteredGroups.value, (group) => {
                _push2(`<div class="space-y-1" data-v-93543a46><div class="text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500" data-v-93543a46>${ssrInterpolate(group.label)}</div><div class="grid grid-cols-5 gap-2" data-v-93543a46><!--[-->`);
                ssrRenderList(group.icons, (ic) => {
                  _push2(`<button type="button"${ssrRenderAttr("title", ic)} class="${ssrRenderClass([{ "ring-2 ring-blue-500": ic === link.icon }, "group w-12 h-12 rounded-md border border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center gap-0.5 text-[9px] text-gray-600 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/30 focus:outline-none focus:ring-2 focus:ring-blue-500"])}" data-v-93543a46>`);
                  _push2(ssrRenderComponent(unref(Icon), {
                    icon: ic,
                    class: "w-5 h-5"
                  }, null, _parent));
                  _push2(`<span class="truncate w-full px-1 group-hover:text-blue-600 dark:group-hover:text-blue-300" data-v-93543a46>${ssrInterpolate(ic.split(":")[1])}</span></button>`);
                });
                _push2(`<!--]--></div></div>`);
              });
              _push2(`<!--]-->`);
            } else {
              _push2(`<div class="text-[11px] text-gray-500 dark:text-gray-400 py-4 text-center" data-v-93543a46><span class="block" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.noMatches"))}</span><a href="https://icon-sets.iconify.design/mdi/" target="_blank" rel="noopener" class="underline block" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.seeAllIcons"))}</a></div>`);
            }
            _push2(`</div><div class="flex justify-between pt-1" data-v-93543a46><button type="button" class="text-[11px] px-2 py-1 rounded bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60" data-v-93543a46>${ssrInterpolate(unref(t)("common.reset"))}</button><span class="text-[10px] text-gray-400 dark:text-gray-500" data-v-93543a46>${ssrInterpolate(unref(t)("settings.siteMeta.iconsCount", { count: totalIcons.value }))}</span></div></div>`);
          } else {
            _push2(`<!---->`);
          }
        }, "body", false, _parent);
        _push(`<input${ssrRenderAttr("value", link.icon)} type="text"${ssrRenderAttr("placeholder", unref(t)("settings.siteMeta.iconPlaceholder"))} class="flex-1 min-w-[140px] h-8 rounded-md px-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-[11px] focus:outline-none focus:ring-2 focus:ring-blue-500" data-v-93543a46><div class="flex flex-wrap items-center gap-1 ms-auto w-full sm:w-auto justify-end" data-v-93543a46><button type="button"${ssrIncludeBooleanAttr(i === 0) ? " disabled" : ""}${ssrRenderAttr("title", unref(t)("settings.siteMeta.moveUp"))}${ssrRenderAttr("aria-label", unref(t)("settings.siteMeta.moveUp"))} class="inline-flex items-center justify-center h-8 w-8 rounded-md text-[11px] font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60 disabled:opacity-40" data-v-93543a46>`);
        _push(ssrRenderComponent(unref(Icon), {
          icon: "mdi:arrow-up",
          class: "text-sm"
        }, null, _parent));
        _push(`</button><button type="button"${ssrIncludeBooleanAttr(i === links.value.length - 1) ? " disabled" : ""}${ssrRenderAttr("title", unref(t)("settings.siteMeta.moveDown"))}${ssrRenderAttr("aria-label", unref(t)("settings.siteMeta.moveDown"))} class="inline-flex items-center justify-center h-8 w-8 rounded-md text-[11px] font-medium bg-gray-100 dark:bg-gray-700/40 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60 disabled:opacity-40" data-v-93543a46>`);
        _push(ssrRenderComponent(unref(Icon), {
          icon: "mdi:arrow-down",
          class: "text-sm"
        }, null, _parent));
        _push(`</button><button type="button" class="inline-flex items-center gap-1 h-8 px-2 rounded-md text-[11px] font-medium bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-900/60" data-v-93543a46>`);
        _push(ssrRenderComponent(unref(Icon), {
          icon: "mdi:delete",
          class: "text-sm"
        }, null, _parent));
      ...
      _push(`<!--]--></ul></div></div><div class="flex items-center gap-3 pt-2" data-v-93543a46><button type="submit"${ssrIncludeBooleanAttr(saving.value) ? " disabled" : ""} class="inline-flex items-center gap-2 h-9 px-4 rounded-md text-sm font-medium bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/40 disabled:opacity-50 border border-blue-200/70 dark:border-blue-800/40" data-v-93543a46>`);
      _push(ssrRenderComponent(unref(Icon), {
        icon: saving.value ? "mdi:loading" : "mdi:content-save",
        class: [saving.value ? "animate-spin" : "", "text-base"]
      }, null, _parent));
      _push(`<span data-v-93543a46>${ssrInterpolate(saving.value ? unref(t)("common.saving") : unref(t)("common.save"))}</span></button></div></form></div>`);
    };
  }
};
...
const BrandingMetaForm = /* @__PURE__ */ _export_sfc(_sfc_main$6, [["__scopeId", "data-v-93543a46"]]);
const pageSize$1 = 10;
const _sfc_main$5 = {
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    async function enrichPostsWithLocales(rows) {
      ...
      const localesByGroup = /* @__PURE__ */ Object.create(null);
      const versionsByGroup = /* @__PURE__ */ Object.create(null);
      if (groupIds.length) {
        const { data: siblings, error } = await supabase.from("posts").select("id, title, slug, locale, status, translation_group_id").in("translation_group_id", groupIds);
        if (!error && siblings) {
          for (const s of siblings) {
            ...
            if (!versionsByGroup[s.translation_group_id]) {
              versionsByGroup[s.translation_group_id] = /* @__PURE__ */ Object.create(null);
            }
      ...
      return rows.map((p) => {
        const translations = p.translation_group_id && versionsByGroup[p.translation_group_id] || (p.locale ? {
          [p.locale]: {
            id: p.id,
            title: p.title,
            slug: p.slug,
        ...
        return {
          ...p,
          locales: p.translation_group_id && localesByGroup[p.translation_group_id] || (p.locale ? [p.locale] : []),
          translations
        };
      });
    }
    ...
    return (_ctx, _push, _parent, _attrs) => {
      const _component_router_link = resolveComponent("router-link");
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "max-w-7xl mx-auto px-4 py-10" }, _attrs))} data-v-07c8b12c>`);
      if (loading.value) {
        _push(`<div class="flex flex-col items-center justify-center min-h-[60vh]" data-v-07c8b12c>`);
        _push(ssrRenderComponent(unref(Icon), {
          icon: "mdi:loading",
          class: "animate-spin text-4xl text-blue-500 mb-4"
        }, null, _parent));
        _push(`<p class="text-gray-700 dark:text-gray-300 text-lg" data-v-07c8b12c>${ssrInterpolate(unref(t)("dashboard.gathering"))}</p></div>`);
      } else {
        _push(`<!--[--><header class="flex items-start justify-between mb-6 flex-wrap gap-4" data-v-07c8b12c><div class="flex items-center gap-2 min-w-0" data-v-07c8b12c>`);
        _push(ssrRenderComponent(unref(Icon), {
          icon: "mdi:monitor-dashboard",
          class: "text-blue-600 dark:text-blue-400 text-3xl"
        }, null, _parent));
        _push(`<h1 class="text-3xl font-bold text-gray-900 dark:text-white break-words" data-v-07c8b12c>${ssrInterpolate(unref(t)("dashboard.title"))}</h1></div><div class="flex flex-wrap items-center gap-2 justify-end w-full sm:w-auto" data-v-07c8b12c>`);
        _push(ssrRenderComponent(_component_router_link, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              ...
              _push2(`<span data-v-07c8b12c${_scopeId}>${ssrInterpolate(unref(t)("posts.newPost"))}</span>`);
            } else {
              return [
                createVNode(unref(Icon), {
                  icon: "mdi:plus",
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
                ...
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(ssrRenderComponent(unref(TabsIndicator), { class: "absolute px-8 left-0 h-[2px] bottom-0 w-[--radix-tabs-indicator-size] translate-x-[--radix-tabs-indicator-position] rounded-full transition-[width,transform] duration-300" }, {
                      default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                        if (_push4) {
                          _push4(`<div class="bg-blue-500 w-full h-full" data-v-07c8b12c${_scopeId3}></div>`);
                        } else {
                          return [
                      ...
                      _push3(ssrRenderComponent(unref(TabsTrigger), {
                        ...
                        default: withCtx((_3, _push4, _parent4, _scopeId3) => {
                          if (_push4) {
                            ...
                            _push4(`<span data-v-07c8b12c${_scopeId3}>${ssrInterpolate(unref(t)("comments.title"))}</span>`);
                            if (stats.value.pendingComments > 0) {
                              _push4(`<span class="inline-flex items-center justify-center min-w-[1.25rem] h-5 px-2 rounded-full text-[11px] font-semibold bg-yellow-100 text-yellow-700 dark:bg-yellow-800/60 dark:text-yellow-200" data-v-07c8b12c${_scopeId3}>${ssrInterpolate(stats.value.pendingComments)}</span>`);
                            } else {
                              _push4(`<!---->`);
                            }
                          } else {
              ...
              _push2(ssrRenderComponent(unref(TabsContent), {
                ...
                default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                  if (_push3) {
                    _push3(`<div class="flex flex-col sm:flex-row sm:items-center gap-3 justify-between" data-v-07c8b12c${_scopeId2}><div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto" data-v-07c8b12c${_scopeId2}>`);
                    ...
                    _push3(`</div><div class="w-full sm:w-80" data-v-07c8b12c${_scopeId2}><div class="flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900/40 focus-within:ring-2 focus-within:ring-blue-500 transition" data-v-07c8b12c${_scopeId2}>`);
                    _push3(ssrRenderComponent(unref(Icon), {
                      icon: "mdi:magnify",
                      class: "ml-3 text-gray-400 text-base"
                    }, null, _parent3, _scopeId2));
                    _push3(`<input${ssrRenderAttr("value", searchQuery.value)} type="text"${ssrRenderAttr("placeholder", unref(t)("posts.searchPlaceholder"))} class="flex-1 h-full bg-transparent px-2 text-sm text-gray-800 dark:text-gray-100 placeholder:text-gray-400 focus:outline-none"${ssrRenderAttr("aria-label", unref(t)("posts.searchPlaceholder"))} data-v-07c8b12c${_scopeId2}>`);
                    if (searchQuery.value) {
                      _push3(`<button type="button" class="mr-1 inline-flex items-center justify-center w-7 h-7 rounded-md text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"${ssrRenderAttr("aria-label", unref(t)("posts.clearSearch"))} data-v-07c8b12c${_scopeId2}>`);
                      _push3(ssrRenderComponent(unref(Icon), {
                        icon: "mdi:close",
                        class: "text-base"
                      }, null, _parent3, _scopeId2));
                                  ...
                                  default: withCtx((_5, _push6, _parent6, _scopeId5) => {
                                    if (_push6) {
                                      _push6(ssrRenderComponent(unref(SelectTrigger), {
                                        ...
                                        default: withCtx((_6, _push7, _parent7, _scopeId6) => {
                                          if (_push7) {
                                            _push7(`<span class="flex items-center gap-1.5 min-w-0 truncate" data-v-07c8b12c${_scopeId6}>`);
                                            _push7(ssrRenderComponent(unref(Icon), {
                                      ...
                                      _push6(ssrRenderComponent(unref(DateRangePickerField), { class: "h-9 w-full items-center select-none bg-white dark:bg-gray-900/40 rounded-md text-gray-800 dark:text-gray-100 border border-gray-300 dark:border-gray-600 px-2.5 text-sm gap-0.5 flex focus-within:ring-2 focus-within:ring-blue-500 focus-within:outline-none" }, {
                                        default: withCtx(({ segments }, _push7, _parent7, _scopeId6) => {
                                          if (_push7) {
                                            ...
                                            _push7(`<!--]--><span class="mx-1 text-gray-400" data-v-07c8b12c${_scopeId6}>\u2013</span><!--[-->`);
                                            ...
                                            _push7(`<!--]--><div class="ml-auto flex items-center gap-0.5 shrink-0" data-v-07c8b12c${_scopeId6}>`);
                                            if (startDate.value || endDate.value) {
                                              _push7(`<button type="button"${ssrRenderAttr("aria-label", unref(t)("common.clear"))} class="inline-flex items-center justify-center h-7 w-7 rounded-md text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" data-v-07c8b12c${_scopeId6}>`);
                                              _push7(ssrRenderComponent(unref(Icon), {
                                                icon: "mdi:close",
                                                class: "w-4 h-4"
                                              }, null, _parent7, _scopeId6));
                                            ...
                                            _push7(ssrRenderComponent(unref(DateRangePickerCalendar), { class: "p-2" }, {
                                              default: withCtx(({ weekDays, grid }, _push8, _parent8, _scopeId7) => {
                                                if (_push8) {
                                                  ...
                                                  _push8(`<div class="flex flex-col space-y-2 sm:flex-row sm:space-x-2 sm:space-y-0" data-v-07c8b12c${_scopeId7}><!--[-->`);
                    ...
                    _push3(`<div class="mt-4 flex items-center justify-center gap-3 text-xs text-gray-600 dark:text-gray-300" data-v-07c8b12c${_scopeId2}><button type="button"${ssrIncludeBooleanAttr(currentPage.value === 1) ? " disabled" : ""} class="inline-flex items-center gap-1 h-8 px-3 rounded-md border border-gray-300 dark:border-gray-600 disabled:opacity-40 hover:bg-gray-50 dark:hover:bg-gray-700/50" data-v-07c8b12c${_scopeId2}>`);
                    _push3(ssrRenderComponent(unref(Icon), {
                      icon: "mdi:chevron-left",
                      class: "text-base"
                    }, null, _parent3, _scopeId2));
                    _push3(` ${ssrInterpolate(unref(t)("common.prev"))}</button><span class="font-medium tabular-nums" data-v-07c8b12c${_scopeId2}>${ssrInterpolate(unref(t)("common.page", { current: currentPage.value, total: totalPages.value || 1 }))}</span><button type="button"${ssrIncludeBooleanAttr(currentPage.value === totalPages.value || totalPages.value === 0) ? " disabled" : ""} class="inline-flex items-center gap-1 h-8 px-3 rounded-md border border-gray-300 dark:border-gray-600 disabled:opacity-40 hover:bg-gray-50 dark:hover:bg-gray-700/50" data-v-07c8b12c${_scopeId2}>${ssrInterpolate(unref(t)("common.next"))} `);
                    _push3(ssrRenderComponent(unref(Icon), {
                      icon: "mdi:chevron-right",
                      class: "text-base"
                    }, null, _parent3, _scopeId2));
                  ...
                  default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    if (_push3) {
                      if (brandingLoaded.value) {
                        _push3(`<div class="grid gap-10 lg:grid-cols-2 items-start" data-v-07c8b12c${_scopeId2}>`);
                        ...
                        _push3(`<div class="text-sm text-gray-500 dark:text-gray-400 py-10 text-center" data-v-07c8b12c${_scopeId2}>${ssrInterpolate(unref(t)("dashboard.loadingSettings"))}</div>`);
                      }
                    } else {
                      return [
                  ...
                  default: withCtx((_2, _push3, _parent3, _scopeId2) => {
                    if (_push3) {
                      if (membersLoaded.value) {
                        ...
                        _push3(`<div class="text-sm text-gray-500 dark:text-gray-400 py-10 text-center" data-v-07c8b12c${_scopeId2}>${ssrInterpolate(unref(t)("dashboard.loadingMembers"))}</div>`);
                      }
                    } else {
                      return [
...
const index = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-07c8b12c"]]);

export { index as default };
//# sourceMappingURL=index-CRVDGkpu.mjs.map
