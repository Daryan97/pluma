    ...
    <template v-else>
      <header class="flex items-start justify-between mb-6 flex-wrap gap-4">
        <div class="flex items-center gap-2 min-w-0">
          ...
          />
          <h1 class="text-3xl font-bold text-gray-900 dark:text-white break-words">{{ t('dashboard.title') }}</h1>
        </div>
        <div class="flex flex-wrap items-center gap-2 justify-end w-full sm:w-auto">
          <router-link
            :to="localePath('/dashboard/new-post')"
      ...
      <TabsRoot class="flex flex-col w-full" v-model="activeTab" default-value="overview">
        <TabsList
        ...
        <TabsContent value="posts" class="space-y-6">
          <div class="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
            ...
            <div class="w-full sm:w-80">
              <div
                class="flex items-center h-9 w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900/40 focus-within:ring-2 focus-within:ring-blue-500 transition"
              >
                <Icon
                  icon="mdi:magnify"
          ...
          >
            <template #filters>
              <PostFilters
                ...
                :has-active-filters="hasActivePostFilters"
                @bulkDelete="askConfirmation(t('posts.deleteSelectedConfirm'), bulkDelete)"
                @clearFilters="clearAllPostFilters"
              >
                <template #status>
                  <SelectRoot v-model="statusFilter" @update:modelValue="applyFilters">
                    <SelectTrigger
                      :class="filterTriggerClass"
                      :aria-label="t('posts.allStatuses')"
                    >
                      <span class="flex items-center gap-1.5 min-w-0 truncate">
                        <Icon icon="mdi:circle-slice-8" class="text-sm opacity-60 shrink-0" />
                        <SelectValue :placeholder="t('posts.allStatuses')" />
                      </span>
                      <Icon icon="radix-icons:chevron-down" class="w-4 h-4 opacity-70 shrink-0" />
                    </SelectTrigger>
                    <SelectPortal>
                      <SelectContent
                          ...
                          <SelectGroup>
                            <SelectItem
                              ...
                              ><SelectItemIndicator
                                ...
                                ><Icon icon="radix-icons:check" /></SelectItemIndicator
                              ><SelectItemText>{{ t('posts.allStatuses') }}</SelectItemText></SelectItem
                            >
                            <SelectSeparator
                            ...
                            <SelectItem
                              ...
                              ><SelectItemIndicator
                                class="absolute left-0 w-8 inline-flex items-center justify-center"
                                ><Icon icon="radix-icons:check" /></SelectItemIndicator
                              ><SelectItemText>{{ t(`posts.status.${s}`) }}</SelectItemText></SelectItem
                            >
                          </SelectGroup>
                        </SelectViewport>
                ...
                <template #locale>
                  <SelectRoot v-model="localeFilter" @update:modelValue="applyFilters">
                    <SelectTrigger
                      :class="filterTriggerClass"
                      :aria-label="t('posts.allLocales')"
                    >
                      <span class="flex items-center gap-1.5 min-w-0 truncate">
                        <Icon icon="mdi:translate" class="text-sm opacity-60 shrink-0" />
                        <SelectValue :placeholder="t('posts.allLocales')" />
                      </span>
                      <Icon icon="radix-icons:chevron-down" class="w-4 h-4 opacity-70 shrink-0" />
                    </SelectTrigger>
                    <SelectPortal>
                        ...
                        <SelectViewport class="p-1">
                          <SelectGroup>
                            ...
                            >
                              ...
                              <SelectItemText>{{ t('posts.allLocales') }}</SelectItemText>
                            </SelectItem>
                            <SelectSeparator
                              class="h-px bg-gray-200 dark:bg-gray-700 my-1"
                            />
                            <SelectItem
                              v-for="loc in filterLocales"
                              :key="loc.code"
                              :value="loc.code"
                              class="text-sm leading-none text-gray-700 dark:text-gray-100 rounded flex items-center h-7 pr-8 pl-8 relative select-none cursor-pointer data-[highlighted]:bg-blue-600 data-[highlighted]:text-white"
                            >
                ...
                <template #category>
                  <SelectRoot v-model="categoryFilter" @update:modelValue="applyFilters">
                    <SelectTrigger
                      :class="filterTriggerClass"
                      :aria-label="t('posts.allCategories')"
                    >
                      <span class="flex items-center gap-1.5 min-w-0 truncate">
                        <Icon icon="mdi:folder-outline" class="text-sm opacity-60 shrink-0" />
                        <SelectValue :placeholder="t('posts.allCategories')" />
                      </span>
                      <Icon icon="radix-icons:chevron-down" class="w-4 h-4 opacity-70 shrink-0" />
                    </SelectTrigger>
                    <SelectPortal>
                          ...
                          <SelectGroup>
                            <SelectItem
                              ...
                              ><SelectItemIndicator
                                class="absolute left-0 w-8 inline-flex items-center justify-center"
                                ><Icon icon="radix-icons:check" /></SelectItemIndicator
                              ><SelectItemText>{{ t('posts.allCategories') }}</SelectItemText></SelectItem
                            >
                            <SelectSeparator
                              class="h-px bg-gray-200 dark:bg-gray-700 my-1"
                            />
                            <SelectItem
                              ...
                              ><SelectItemIndicator
                                class="absolute left-0 w-8 inline-flex items-center justify-center"
                                ><Icon icon="radix-icons:check" /></SelectItemIndicator
                              ><SelectItemText>{{ c.name }}</SelectItemText></SelectItem
                            >
                          </SelectGroup>
                        </SelectViewport>
                        <SelectScrollDownButton
                ...
                <template #author>
                  <SelectRoot v-model="authorFilter" @update:modelValue="applyFilters">
                    <SelectTrigger
                      :class="filterTriggerClass"
                      :aria-label="t('posts.allAuthors')"
                    >
                      <span class="flex items-center gap-1.5 min-w-0 truncate">
                        <Icon icon="mdi:account-outline" class="text-sm opacity-60 shrink-0" />
                          ...
                          <SelectGroup>
                            <SelectItem
                              ...
                              ><SelectItemIndicator
                                class="absolute left-0 w-8 inline-flex items-center justify-center"
                                ><Icon icon="radix-icons:check" /></SelectItemIndicator
                              ><SelectItemText>{{ t('posts.allAuthors') }}</SelectItemText></SelectItem
                            >
                            <SelectSeparator
                              class="h-px bg-gray-200 dark:bg-gray-700 my-1"
                            />
                ...
                <template #date>
                  ...
                  >
                    <DateRangePickerField
                      v-slot="{ segments }"
                      class="h-9 w-full items-center select-none bg-white dark:bg-gray-900/40 rounded-md text-gray-800 dark:text-gray-100 border border-gray-300 dark:border-gray-600 px-2.5 text-sm gap-0.5 flex focus-within:ring-2 focus-within:ring-blue-500 focus-within:outline-none"
                    >
                      <Icon
                      ...
                      <div class="ml-auto flex items-center gap-0.5 shrink-0">
                        ...
                        <DateRangePickerTrigger
                          class="inline-flex items-center justify-center h-7 w-7 rounded-md text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                        >
                          <Icon icon="radix-icons:calendar" class="w-4 h-4" />
                        </DateRangePickerTrigger>
                      </div>
                    </DateRangePickerField>
          ...
          <div
            class="mt-4 flex items-center justify-center gap-3 text-xs text-gray-600 dark:text-gray-300"
          >
            <button
              type="button"
            ...
            <span class="font-medium tabular-nums">{{
              t('common.page', { current: currentPage, total: totalPages || 1 })
            }}</span>
            <button
              type="button"
              @click="changePage(currentPage + 1)"
        ...
        >
          <PendingComments
            ...
            @delete="
              (id) =>
                askConfirmation(t('comments.deleteConfirm'), () => deletePendingComment(id))
            "
          />
        </TabsContent>
        ...
        <TabsContent value="settings" v-if="role === 'admin'" class="space-y-10">
          <template v-if="brandingLoaded">
            <div class="grid gap-10 lg:grid-cols-2 items-start">
              ...
              <FooterCreditsSettings class="self-stretch lg:col-span-2" />
...
<script setup>
definePageMeta({ requiresAuth: true, requiresAuthorOrAdmin: true, ssr: false })
...
async function fetchPostLocalesInUse() {
  ...
  if (
    ...
    !postLocalesInUse.value.includes(localeFilter.value)
  ) {
    localeFilter.value = "all";
  }
}
...
const filterTriggerClass =
  "w-full inline-flex h-9 items-center justify-between gap-2 rounded-md px-2.5 text-sm bg-white dark:bg-gray-900/40 border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 data-[placeholder]:text-gray-400";
...
function isTabAllowed(tab) {
  ...
  if (tab === "comments") return role.value === "admin" || role.value === "author";
