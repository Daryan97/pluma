  ...
  <div class="min-h-screen">
      ...
      <div class="max-w-6xl mx-auto px-4 pt-20 pb-16 lg:pt-28 lg:pb-24 relative">
        <div class="flex flex-col items-center text-center gap-8">
          <div class="space-y-5 max-w-3xl">
            <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-gray-900 via-gray-800 to-gray-900 dark:from-white dark:via-gray-100 dark:to-white">{{ t('archive.title') }}</h1>
            <p class="text-lg md:text-xl text-gray-600 dark:text-gray-300 leading-relaxed">{{ t('archive.description') }}</p>
          </div>
          <div v-if="showSearch" class="w-full max-w-xl">
            ...
            >
              ...
              <span class="flex-1 text-left text-sm text-gray-500 dark:text-gray-400">{{ t('home.searchPlaceholder') }}</span>
    ...
    <main class="relative -mt-4">
      <div class="max-w-6xl mx-auto px-4 py-12">
        <h2 class="sr-only">{{ t('archive.archivedPosts') }}</h2>
          ...
          <article v-for="post in posts" :key="post.id" class="group relative bg-white/95 dark:bg-gray-800/80 backdrop-blur-sm border border-gray-200/80 dark:border-gray-700/70 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
            ...
            <div class="p-6 md:p-8">
              <div class="flex flex-wrap items-center gap-3 text-[11px] font-medium mb-4">
                <router-link :to="localePath(`/archive/category/${post.category?.slug || 'uncategorized'}`)" class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/50">
                  <Icon icon="mdi:folder" class="text-sm" />
                ...
                <router-link :to="localePath(`/author/${getAuthorUsername(post.author)}`)" class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-gray-100 dark:bg-gray-700/40 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700/60">
                  <Icon icon="mdi:account" class="text-sm" />
                  {{ getAuthorName(post.author) }}
                </router-link>
              </div>
              <router-link :to="localePath(`/archive/post/${post.slug}`)" class="group/title block mb-3">
              ...
              <div class="flex items-center justify-between">
                <router-link :to="localePath(`/archive/post/${post.slug}`)" class="inline-flex items-center gap-2 h-9 px-4 rounded-md text-sm font-medium bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-900/60 focus:outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-gray-500">
...
function getAuthorName(author){ if(!author) return t('common.unknownAuthor'); return author.display_name || author.username }
function getAuthorUsername(author){ if(!author) return 'unknown'; return author.username || author.display_name || 'Unknown' }
function getCategoryName(category){ return category ? category.name : t('common.uncategorized') }
