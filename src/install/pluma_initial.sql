...
do $$ begin
  if not exists (select 1 from pg_type where typname = 'user_role') then
    create type public.user_role as enum ('admin','author','reader','disabled');
  end if;
end $$;
...
create or replace function public.is_self(row_id uuid)
returns boolean
language sql stable
set search_path = ''
as $$ select auth.uid() = row_id; $$;

create or replace function public.is_author(uid uuid)
returns boolean
language sql stable
set search_path = ''
as $$
...
create or replace function public.can_manage_own_post(author uuid)
returns boolean
language sql stable
set search_path = ''
as $$
...
declare
  is_first boolean;
begin
  perform pg_advisory_lock(42000001);
  select not exists (
  ...
  values (
    ...
    case when is_first then 'admin'::public.user_role else 'reader'::public.user_role end
  )
...
create policy "categories_insert_author_admin"
on public.categories for insert
to authenticated
with check (public.is_author() or public.is_admin());

create policy "categories_update_author_admin"
on public.categories for update
to authenticated
using (public.is_author() or public.is_admin())
with check (public.is_author() or public.is_admin());

create policy "categories_delete_author_admin"
...
create policy "posts_read_published_or_authors"
on public.posts for select
to public
using (status = 'published' or public.is_author());

create policy "posts_insert_authors"
on public.posts for insert
to authenticated
with check (public.is_author());

create policy "posts_update_admin_or_owner_author"
on public.posts for update
to authenticated
using (public.is_admin() or public.can_manage_own_post(author_id))
with check (public.is_admin() or public.can_manage_own_post(author_id));

create policy "posts_delete_admin_or_owner_author"
on public.posts for delete
to authenticated
using (public.is_admin() or (author_id = auth.uid() and public.is_author()));
...
create policy "comments_delete_self_or_author"
on public.comments for delete
to authenticated
using (public.is_self(author_id) or public.is_author());
...
create policy "settings_insert_install_flag"
on public.settings for insert
to anon
with check (
  key = 'installation'
  and not exists (select 1 from public.settings where key = 'installation')
);

-- One-time anonymous insert of branding (must be before installation completes)
create policy "settings_insert_branding_initial"
on public.settings for insert
to anon
with check (
  key = 'branding'
  and not exists (select 1 from public.settings where key = 'branding')
  and not exists (select 1 from public.settings where key = 'installation')
);
...
create policy "buckets_list_admin"
on storage.buckets for select
using (true); -- (Use your preferred rule; earlier you had admin-only.)

create policy "buckets_create_admin"
...
create policy "objects_read_all"
...
create policy "thumbnails_insert_author"
on storage.objects for insert
to authenticated
with check (bucket_id = 'post-thumbnails' and (public.is_author() or public.is_admin()));

create policy "thumbnails_update_author_or_admin"
on storage.objects for update
to authenticated
using     (bucket_id = 'post-thumbnails' and (public.is_author() or public.is_admin()))
with check(bucket_id = 'post-thumbnails' and (public.is_author() or public.is_admin()));

create policy "thumbnails_delete_admin_only"
on storage.objects for delete
to authenticated
using (bucket_id = 'post-thumbnails' and public.is_admin());
...
on storage.objects for insert
to authenticated
with check (bucket_id = 'profile-avatar' and (storage.foldername(name))[1] = auth.uid()::text);

create policy "avatars_update_owner"
on storage.objects for update
to authenticated
using     (bucket_id = 'profile-avatar' and (storage.foldername(name))[1] = auth.uid()::text)
with check(bucket_id = 'profile-avatar' and (storage.foldername(name))[1] = auth.uid()::text);

create policy "avatars_delete_owner_or_admin"
on storage.objects for delete
to authenticated
using (bucket_id = 'profile-avatar' and ( (storage.foldername(name))[1] = auth.uid()::text or public.is_admin() ));
