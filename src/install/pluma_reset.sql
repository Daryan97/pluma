...
do $$
begin
  -- Drop auth trigger (if auth.users exists)
  if exists (
  ...
  if exists (
    ...
    execute 'drop policy if exists "objects_read_all"                  on storage.objects';
    ...
    execute $sql$
      ...
      where bucket_id in ('post-thumbnails','profile-avatar','branding')
    $sql$;
  end if;
  ...
  if exists (
    ...
    execute 'drop policy if exists "buckets_list_admin"   on storage.buckets';
    ...
    execute $sql$
      ...
      where id in ('post-thumbnails','profile-avatar','branding')
    $sql$;
  end if;
end$$;
...
do $$
begin
  -- --- Policies (guarded per-table) ---
  if exists (select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace
             where n.nspname='public' and c.relname='categories') then
  ...
  if exists (select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace
             where n.nspname='public' and c.relname='posts') then
    execute 'drop policy if exists "posts_read_published_or_authors"   on public.posts';
  ...
  if exists (select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace
             where n.nspname='public' and c.relname='comments') then
    execute 'drop policy if exists "comments_read_all"             on public.comments';
  ...
  if exists (select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace
             where n.nspname='public' and c.relname='settings') then
    execute 'drop policy if exists "settings_read_all"             on public.settings';
